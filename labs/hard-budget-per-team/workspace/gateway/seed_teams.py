#!/usr/bin/env python3
"""Create the two teams this lab is about, and a key for each, the first
time the proxy comes up -- and do nothing on every boot after that.

Idempotent by construction, not by a lock file: each team is given a fixed
`team_id` (not a random one), so `GET /team/info?team_id=...` tells us
on every boot whether that team already exists. Same for its key -- once
one is minted, its value is written to keys.json, and a restart reuses it
(after confirming it still authenticates) rather than minting a second one
the learner would have to go find.
...
TEAMS = [
    ...
    {
        "team_id": "team-support-desk",
        "team_alias": "support-desk",
        "max_budget": 100.0,
        "models": ["assistant"],
    },
]
...
def main():
    ...
    for team in TEAMS:
        ...
        out[team["team_id"]] = {"team_alias": team["team_alias"], "key": key, "max_budget": team["max_budget"]}
