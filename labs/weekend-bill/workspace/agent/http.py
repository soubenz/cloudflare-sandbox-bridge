...
retrying. Every outbound call the agent makes goes through here, which is
the point of putting the policy here: callers get a uniform contract. A
value returned from this function worked. An exception out of it is not
going to work, and has already been given MAX_ATTEMPTS chances.
"""
...
def _post_once(url, payload, timeout):
    ...
    try:
        ...
        if err.code >= 500:
            ...
            # error body does not say what it cost to find that out.
